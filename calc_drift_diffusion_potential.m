...
if strcmp(how_many_Ds, '2D')
    ...
    F = zeros(n1+1,n2+1,2,1); % row 1 and 2: drift in rv1 and rv2, respectively
        ...
        for j=0:n2 % rv2
            ...
            if ~isempty(x)
                ...
                for k=1:length(x)
                    ...
                    delta_rv = Delta_rv - ones(length(x),1)*mean(Delta_rv,1);
                    delta_rv_tensor_delta_rv(:,:,k) = delta_rv(k,:)'*delta_rv(k,:);
                end
                
                F(i+1,j+1,:,1) = mean(Delta_rv./delta_t,1);
                D(:,:,i+1,j+1) = 0.5 * mean(delta_rv_tensor_delta_rv./delta_t,3);
            else
...
elseif strcmp(how_many_Ds,'1D')
    ...
    for i=0:N
        ...
        % Calculating Peclet number (Unitless number to compare drift and diffusion)
    ...
    if plot_DDE_stuff
        ...
        title(['Histogram of ' data_title ' from ' sim_title], 'FontSize', 15);
        ...
        X = 1:min(1e7,length(t)); % there is reall no need to plot all of t!!!
        figure,
        subplot(1,2,1), plot(X,t(X));
        xlabel('n'); ylabel('t(n)');
        title(['Mean delta t = mean(t(n+1)-t(n)) = ' num2str(delta_t)], 'FontSize', 15);
        ...
        legend('Hist for simulated t(n+1)-t(n)', ...
            ['Hist for exp distributed data with mean' num2str(delta_t)], ...
            'Location','Best');
        xlabel('delta t'); ylabel('Natural logarithm of frequency');
    end
end
